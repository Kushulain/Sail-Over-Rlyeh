...
Shader "Hidden/Internal-ScreenSpaceShadowsCustom" {
Properties {
...
#define UNITY_CASCADE_BLEND_DISTANCE 0.5
...
//	float4 wpos = mul(unity_CameraToWorld, float4(vpos,1));
//	fixed4 cascadeWeights = GET_CASCADE_WEIGHTS(wpos, vpos.z);
//	//cascadeWeights = float4(0.25,0.25,0.25,0.25);
//	//cascadeWeights.x *= 1.01;
...
//	if (dir.y < 0.0 || start.y < VOLUMETRIC_SHADOW_HEIGHT)
	{
//		if (start.y > VOLUMETRIC_SHADOW_HEIGHT)// 
//		{
//			dir *= 20.0;
			start += dir * ((min(0.0,VOLUMETRIC_SHADOW_HEIGHT-start.y))/(dir.y));
			start.w = 1.0;
//		}

		dir = float4(wpos-start.xyz,1.0);

//		if ((start.y+dir.y) > VOLUMETRIC_SHADOW_HEIGHT)
//		{
			dir *=   (min(dir.y,VOLUMETRIC_SHADOW_HEIGHT-start.y))/(dir.y);
//			dir *=   (VOLUMETRIC_SHADOW_HEIGHT-start.y)/(dir.y);
//			dir *= 0.02;
//		}


	//	if (wpos.y > VOLUMETRIC_SHADOW_HEIGHT)
	//	{
	//		wpos.xyz /= wpos.y / VOLUMETRIC_SHADOW_HEIGHT;
	//	}

	//	if (_WorldSpaceCameraPos.y > VOLUMETRIC_SHADOW_HEIGHT)
	//	{
	//		_WorldSpaceCameraPos.xyz /= _WorldSpaceCameraPos.y / VOLUMETRIC_SHADOW_HEIGHT;
	//	}
	//
